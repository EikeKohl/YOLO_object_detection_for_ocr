...
def plot_image_from_array(img_array,
    ...
    if plot_grids:
        ...
        nx = abs(int(float(ax.get_xlim()[1] - ax.get_xlim()[0]) / float(number_of_grids)))
        ny = abs(int(float(ax.get_ylim()[1] - ax.get_ylim()[0]) / float(number_of_grids)))
        ...
        for j in range(ny):
            ...
            for i in range(nx):
                x = number_of_grids / 2. + float(i) * number_of_grids
